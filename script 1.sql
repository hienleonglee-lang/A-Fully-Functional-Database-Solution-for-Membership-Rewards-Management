...
--------------------------------------------------------------------------------
-- User Transaction:
-- Compare the current tier of a customer with the next tier to determine upgrade eligibility.
-- This includes calculating how many more points are needed, the discount difference,
-- and the change in renewal fee. This helps staff or the customer service system
-- advise members on their upgrade progress.
...
BEGIN
    ...
    ELSE
        DBMS_OUTPUT.PUT_LINE('No Manully Updated tier needed - customer already at appropriate tier, Automatically Upgraded Tier Successfully.');
    END IF;
EXCEPTION
    WHEN OTHERS THEN
...
-- output:No Manully Updated tier needed - customer already at appropriate tier, Automatically Upgraded Tier Successfully.
